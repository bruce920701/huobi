##REST API
###简介
* [请求过程](REST-Trade-API-Method)<br>
* [代码示例](REST-Code-Instance)<br>
* [错误代码](REST-Error-Code)<br>

###REST交易API
* [获取个人资产信息](REST-get_account_info)<br>
* [获取所有正在进行的委托](REST-get_orders)<br>
* [获取委托详情](REST-order_info)<br>
* [买入](REST-buy)<br>
* [卖出](REST-sell)<br>
* [买入(市价单)](REST-buy_market)<br>
* [卖出(市价单)](REST-sell_market)<br>
* [取消委托单](REST-cancel_order)<br>
* [修改订单](REST-modify_order)<br>
* [查询个人最新10条成交订单](REST-get_new_deal_orders)<br>
* [根据trade_id查询oder_id](REST-get_order_id_by_trade_id)<br>
* [提币BTC/LTC](REST-withdraw_coin)<br>
* [取消提币BTC/LTC](REST-cancel_withdraw_coin)<br>
* [查询提币BTC/LTC](REST-get_withdraw_coin_result)<br> 
* [账户内转账](REST-transfer)<br> 

###REST行情API
* [分时行情图](REST-Interval)<br>
* [实时行情图](REST-Candlestick-Chart)<br>
* [买卖盘和实时成交数据](REST-Order-Book-and-TAS)<br>

##FIX行情交易API（V2）（仅支持人民币行情）
* [火币FIX API简介](FIX-v2-General)<br>
* [会话管理](FIX-v2-Session)<br>
* [交易接口](FIX-v2-Trading Interface)<br>
* [行情接口](FIX-v2-Market-Data-Interface)<br>
* [异常返回消息](FIX-v2-Reject)<br>
* [API参考实例](FIX-v2-API-instances)<br>

##WebSocket行情API
* [引言](WebSocket-General)<br>
* [行情API使用说明](WebSocket-Market-API-introduction)<br>
* [数据格式](WebSocket-Data-formats)<br>
* [API参考实例](WebSocket-API-instances)<br>